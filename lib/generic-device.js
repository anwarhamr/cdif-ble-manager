...
GenericDevice.prototype._updateDeviceSpec = function() {
  var _this = this;
  var hasError = false;
  var spec = {};
...
function InvokeCall(serviceUUID, characteristicUuid, device) {
  var _this = this;
  ...
  this.invoke = function(args, callback) {
    ...
    _this.device.writeUInt8Characteristic(serviceUUID, characteristicUuid, value, function(err, data) {
      callback(err, data);
    });
  };
}
...
GenericDevice.prototype.setupDeviceCalls = function(spec) {
  ...
  for (var i in spec.device.serviceList) {
    ...
    for (var j in actionList) {
      ...
      for (var k in action.argumentList) {
        var argumentName = k;
        var argument = action.argumentList[k];
        if (argument.direction === 'in') {
          ...
          var call = new InvokeCall(serviceUUID, characteristicUuid, this.device);
          if (stateVar.dataType === 'uint8') {
          ...
          var characteristicUuid = stateVar.uuid;
          if (stateVar.dataType === 'uint8') {
            this.setAction(serviceID, actionName, function(args, callback) {
              _this.device.readUInt8Characteristic(serviceUUID, characteristicUuid, function(err, data) {
                callback(err, data);
              });
            });
          }
