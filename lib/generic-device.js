  ...
  for (var i in spec.device.serviceList) {
    ...
    for (var j in actionList) {
      ...
      if (argument.direction === 'in') {
        call = new InvokeObject(serviceUUID, characteristicUuid, this.device, false, argName, stateVar.dataType);
      } else {
        call = new InvokeObject(serviceUUID, characteristicUuid, this.device, true, argName, stateVar.dataType);
      }
      // TODO: implement subscribe / unsubscribe calls and onEvent callback
      this.setAction(serviceID, actionName, call.invoke);
    }

    var serviceStateTable = service.serviceStateTable;
    var events = [];
    for (var name in serviceStateTable) {
      if (serviceStateTable[name].sendEvents === true) {
        var eventInfo = new Object();
        eventInfo.uuid = serviceStateTable[name].uuid;
        eventInfo.name = name;
    ...
    console.log('setup subscription for ' + serviceID);
